...
- `depends_on: gitlab-mcp` — the GitLab MCP server container is started automatically and reachable at `http://gitlab-mcp:3002/mcp` from inside the opencode container. OpenCode's `mcp.gitlab` config (in `~/.config/opencode/opencode.jsonc`) points at this URL.
...
- `mr-review` skill — workflow for resolving GitLab MR review comments using `gitlab_*` MCP tools.
- `dotfiles` repo `.agents/skills/original/environment/SKILL.md` is the canonical source for this file; keep it in sync.